...
/**
 * a utility class to help with creating tables, regardless of their schema
 */
public class CreateTableUtils {
     ...
     * @throws IOException
     */
    public static boolean createTable(Configuration conf, String tablePath,
            byte[][] colFams) throws IOException {
    ...
    /**
     * reads data from a CSV text file and constructs Trade objects, one per
     * line. Each trade is added to a List<Trade>, and the list is returned.
    ...
    public static List<Trade> getDataFromFile(String filePath) throws IOException {
        List<Trade> trades = new ArrayList<Trade>(); // This is the list of
        // Trades to populate from
        // the input file and
        // return.
        ...
        while ((currentLine = br.readLine()) != null) {
            ...
            tokens = currentLine.split("\\s*,\\s*"); // Split on comma
            // boundaries, stripping
            // white space.
            if ((tokens != null) && (tokens.length == 4)) {
                trades.add(new Trade(tokens[0], Float.parseFloat(tokens[1]), Long
                        .parseLong(tokens[2]), Long.parseLong(tokens[3])));
            } else {
