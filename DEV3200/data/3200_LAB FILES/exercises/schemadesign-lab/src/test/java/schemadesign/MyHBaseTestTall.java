...
public class MyHBaseTestTall {
	...
	String symbol = "GOOG";
	Long startDate = dateArgFormat.parse("20131010", new ParsePosition(0)).getTime();
	Long stopDate = dateArgFormat.parse("20131011", new ParsePosition(0)).getTime();
	List<Trade> retrievedTradeSet = tradeDao.getTradesByDate(symbol, startDate,
		stopDate);
	// Print the results
	...
	symbol = "AMZN";
	startDate = dateArgFormat.parse("20131010", new ParsePosition(0)).getTime();
	stopDate = dateArgFormat.parse("20131011", new ParsePosition(0)).getTime();
	retrievedTradeSet = tradeDao.getTradesByDate(symbol, startDate, stopDate);
	// Print the results
	LookupTrades.printTrades(retrievedTradeSet);
	assertEquals(retrievedTradeSet.size(), 1);
	...
	symbol = "AMZN";
	startDate = dateArgFormat.parse("20131009", new ParsePosition(0)).getTime();
	stopDate = dateArgFormat.parse("20131009", new ParsePosition(0)).getTime();
	retrievedTradeSet = tradeDao.getTradesByDate(symbol, startDate, stopDate);
	assertEquals(0,retrievedTradeSet.size());
	
	symbol = "CSCO";
	startDate = dateArgFormat.parse("20131009", new ParsePosition(0)).getTime();
	stopDate = dateArgFormat.parse("20131010", new ParsePosition(0)).getTime();
	retrievedTradeSet = tradeDao.getTradesByDate(symbol, startDate, stopDate);
	// Print the results
	LookupTrades.printTrades(retrievedTradeSet);
	assertEquals(retrievedTradeSet.size(), 1);
	...
	// String tradeSymbol, Float tradePrice, Long tradeVolume, Long tradeTime
