...
public class TradeDAOTall implements TradeDAO {
   ...
   // public static final String userdirectory = System.getProperty("user.home");
    ...
    public List<Trade> scanTradesWithFilter(Filter filter) throws IOException {
        Scan scan = mkScan();
	// TODO 2 finish code
        // TODO 2 set input filter on scan object
        // call table.getScanner()
        ResultScanner scanner = null; // TODO 2 table.getScanner(scan);
        System.out.println("************************************");
        System.out.println("Scan results for Table with filters:");
        List<Trade> trades = new ArrayList<Trade>();
	// for each scanResult call  resultToString
        // TODO 2 for (Result scanResult : scanner) {
        System.out.println("Scan Result ");
	 // TODO 2   resultToString(scanResult);
        //   createTradeFromResult(trades, scanResult);
        //}
        scanner.close();
        return trades;
    ...
    private static void resultToString(Result result) {
        System.out.print(" RowKey " + Bytes.toString(result.getRow()));
        for (KeyValue kv : result.raw()) {
            if (kv.getValue().length > 0) {
                String qualifier = Bytes.toString(kv.getQualifier());
                // System.out.println(" KeyValue to String " + kv);
                if (qualifier.equals("price")) {
                    byte[] priceBytes = kv.getValue();
                    Float price = Bytes.toLong(priceBytes) / 100f;
                    System.out.print(" , price : " + price);
                }
                if (qualifier.equals("vol")) {
                    System.out.print(", vol : " + Bytes.toLong(kv.getValue()));
                }
            }
        }
        System.out.println(" ");
