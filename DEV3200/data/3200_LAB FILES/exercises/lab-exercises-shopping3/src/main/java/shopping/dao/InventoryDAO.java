...
public class InventoryDAO {
    ...
    public Result checkoutWithIncrement(String stockId, String cartId, long amt)
            throws Exception {
        // TODO 1 finish checkout code
        Result result = null;
        ...
        if (currentQuantity - amt < 0) {
            ...
            // TODO 1  Create Increment object with stockId key
            Increment inc = null;
			// TODO add column to increment object for inventory quantity - amount in shopping cart
            // TODO add column to increment object for user name and shopping cart amount
            result = table.increment(inc);
            System.out.println("--checkout increment: Changed " + stockId
	...
	public boolean checkout(String stockId, String cartId, long amt)
		...
		if (newQuantity < 0)
			...
			byte[] userCol = Bytes.toBytes(cartId);
			Put put = mkPut(stockId, newQuantity);
			put.add(STOCK_CF, userCol, Bytes.toBytes(amt));
			ok = table.checkAndPut(Bytes.toBytes(stockId), STOCK_CF,
					QUANTITY_COL, Bytes.toBytes(currentQuantity), put);
