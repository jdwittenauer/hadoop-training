...
public class StockReducerTest {
 ...
 //   @Test
	...
	Cell minKeyValue = put.get(COLUMN_FAMILY_STATS, COL_QUALIFIER_MIN).get(0);
	String min = Bytes.toString(minKeyValue.getValue());
	System.out.println(" output min " + min);
	String max = Bytes.toString(CellUtil.cloneValue(put.get(COLUMN_FAMILY_STATS, COL_QUALIFIER_MAX)
		.get(0)));
	System.out.println(" output max " + max);
	String mean = Bytes.toString(CellUtil.cloneValue(put.get(COLUMN_FAMILY_STATS, COL_QUALIFIER_MEAN)
		.get(0)));
