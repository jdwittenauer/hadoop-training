...
/**
 * Persist data into HBase with MapReduce.
 * Depending on command line parameters output data to table using TableOutputFormat or save to HFile. 
 * To save the data to an HTable invoke this job with the following arguments (replace user19 with your user name):
 * /user/user19/hly_temp /user/user19/input
 *
 */
public class BulkLoadMapReduce {
	...
	public static Job createSubmittableJob(Configuration conf, String[] args)
	    ...
	    if (args.length == 0) {
	    	System.out.println("Usage for setup: java -cp `hbase classpath`:./lab-solutions-1.0.jar bulkload.BulkloadMapReduce <TABLE> <DIR PATH to the input file>   ");
	    	System.out.println(" java -cp `hbase classpath`:./lab-solutions-1.0.jar  bulkload.BulkLoadMapReduce /user/user19/hly_temp /user/user19/input");
	    } 
